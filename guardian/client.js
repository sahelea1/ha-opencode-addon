...
(function () {
  ...
  bar.innerHTML = [
    ...
    "  </div>",
    '  <button class="gcfg-btn gcfg-confirm" type="button">Approve</button>',
    '  <button class="gcfg-btn gcfg-revert"  type="button">Reset</button>',
    "</div>",
  ].join("\n");
    ...
    fetch(API_BASE + "status", { cache: "no-store" })
      ...
      .then(function (d) {
        if (d.status === "pending") {
          ...
          bar.querySelector(".gcfg-time").textContent = d.remainingMs == null ? "Review" : fmt(d.remainingMs);
          ...
          if (d.remainingMs != null && d.remainingMs < 120000) bar.classList.add("urgent");
  ...
  window.__gcfgRevert = function () {
    if (!confirm("Reset all pending changes?\n\nThis restores the last approved config, deletes newly created included files, and triggers a Home Assistant reload.")) {
      return;
    }
    fetch(API_BASE + "revert", { method: "POST" })
      ...
      .then(function (d) {
        if (d.ok) {
          showToast("Reset complete - HA reloading", "reverted");
          bar.classList.remove("show", "urgent");
          document.body.style.paddingTop = "";
        }
      })
