 ...
 * 5. Manage backup/restore lifecycle with 10-min approval window
 * 6. On timeout or crash-recovery: reset config and reload HA
 */
...
const STAGING_BACKUP_DIR = path.join(DATA_DIR, "last-known-good.staging");
const PREVIOUS_BACKUP_DIR = path.join(DATA_DIR, "last-known-good.previous");
...
function directoryHasEntries(dir) {
  try {
    return fs.existsSync(dir) && fs.readdirSync(dir).some((name) => name !== ".guardian-keep");
  } catch (e) {
    return false;
  }
}
...
function readBaselineManifest() {
  try {
    ...
    console.error("[guardian] Failed to read baseline manifest:", e.message);
    return null;
  }
}

function hasValidBaseline() {
  const manifest = readBaselineManifest();
  if (!manifest || !manifest.hash || !directoryHasEntries(BACKUP_DIR)) return false;
  const backupHash = getTreeHash(BACKUP_DIR);
  return backupHash === manifest.hash;
}

function ensureBaselineManifest() {
  if (hasValidBaseline()) return true;
  if (fs.existsSync(BASELINE_FILE)) return false;
  if (!directoryHasEntries(BACKUP_DIR)) return false;

  const backupHash = getTreeHash(BACKUP_DIR);
  if (!backupHash) return false;

  try {
    const tmp = BASELINE_FILE + ".tmp";
    fs.writeFileSync(
      tmp,
      JSON.stringify({ migratedAt: new Date().toISOString(), hash: backupHash }, null, 2)
    );
    fs.renameSync(tmp, BASELINE_FILE);
    console.log("[guardian] Migrated existing backup into verified baseline manifest");
    return true;
  } catch (e) {
    console.error("[guardian] Failed to migrate baseline manifest:", e.message);
    return false;
  }
}

function recoverBackupTransaction() {
  try {
    const manifest = readBaselineManifest();
    const manifestHash = manifest && manifest.hash;
    const backupHash = directoryHasEntries(BACKUP_DIR) ? getTreeHash(BACKUP_DIR) : null;
    const previousHash = directoryHasEntries(PREVIOUS_BACKUP_DIR)
      ? getTreeHash(PREVIOUS_BACKUP_DIR)
      : null;

    if (previousHash) {
      if (manifestHash && previousHash === manifestHash && backupHash !== manifestHash) {
        fs.rmSync(BACKUP_DIR, { recursive: true, force: true });
        fs.renameSync(PREVIOUS_BACKUP_DIR, BACKUP_DIR);
        console.log("[guardian] Recovered previous backup after interrupted baseline update");
      } else if (backupHash === manifestHash) {
        fs.rmSync(PREVIOUS_BACKUP_DIR, { recursive: true, force: true });
      }
    }

    fs.rmSync(STAGING_BACKUP_DIR, { recursive: true, force: true });
    fs.rmSync(BASELINE_FILE + ".tmp", { force: true });
  } catch (e) {
    console.error("[guardian] Backup transaction recovery failed:", e.message);
  }
}
...
function getTreeHash(rootDir) {
  try {
    ...
    function walk(dir) {
      for (const name of fs.readdirSync(dir).sort()) {
        ...
        if (stat.isSymbolicLink()) {
          hash.update(`L ${relativePath}\0${fs.readlinkSync(fullPath)}\n`);
          continue;
        }

        if (stat.isFile()) {
          hash.update(`F ${relativePath}\0${stat.size}\0`);
    ...
    console.error("[guardian] Failed to hash config tree:", e.message);
    return null;
  }
}

function getConfigHash() {
  return getTreeHash(CONFIG_DIR);
}
...
function createBackup() {
  ...
  try {
    fs.rmSync(STAGING_BACKUP_DIR, { recursive: true, force: true });
    fs.mkdirSync(STAGING_BACKUP_DIR, { recursive: true });
    execSync(
      `rsync -a --delete ${RSYNC_EXCLUDES} ${CONFIG_DIR}/ ${STAGING_BACKUP_DIR}/`,
      { timeout: 60000 }
    );
    if (!directoryHasEntries(STAGING_BACKUP_DIR)) {
      throw new Error("backup directory is empty after rsync");
    }
    const backupHash = getTreeHash(STAGING_BACKUP_DIR);
    if (!backupHash) {
      throw new Error("backup hash failed after rsync");
    }

    fs.writeFileSync(
      tmp,
      JSON.stringify({ createdAt: new Date().toISOString(), hash: backupHash }, null, 2)
    );

    fs.rmSync(PREVIOUS_BACKUP_DIR, { recursive: true, force: true });
    if (fs.existsSync(BACKUP_DIR)) {
      fs.renameSync(BACKUP_DIR, PREVIOUS_BACKUP_DIR);
    }
    fs.renameSync(STAGING_BACKUP_DIR, BACKUP_DIR);
    fs.renameSync(tmp, BASELINE_FILE);
    fs.rmSync(PREVIOUS_BACKUP_DIR, { recursive: true, force: true });
    ...
    try {
      if (fs.existsSync(PREVIOUS_BACKUP_DIR)) {
        fs.rmSync(BACKUP_DIR, { recursive: true, force: true });
        fs.renameSync(PREVIOUS_BACKUP_DIR, BACKUP_DIR);
      }
    } catch (rollbackError) {
      console.error("[guardian] Backup rollback failed:", rollbackError.message);
    }
...
function restoreBackup() {
  try {
    if (!ensureBaselineManifest()) {
      console.error("[guardian] No valid backup baseline found, cannot restore");
      return false;
    }
    execSync(
...
function getChangedFiles() {
  try {
    ...
    const output = execSync(
      `rsync -n -a --delete ${RSYNC_EXCLUDES} --out-format='%n' ${CONFIG_DIR}/ ${BACKUP_DIR}/ 2>/dev/null || true`,
      { encoding: "utf8", timeout: 10000 }
    );
    return output
      .split("\n")
      .filter((l) => l.trim() && !l.endsWith("/"))
      .map((f) => f.trim().replace(/^\*deleting\s+/, "deleted: "))
      .slice(0, 30);
  } catch (e) {
    return [];
  }
...
function confirmChanges() {
  console.log("[guardian] User approved changes");
  if (!createBackup()) {
    return { ok: false, message: "Approval failed because the new backup baseline could not be saved." };
  }
  state.status = "idle";
  state.deadline = null;
  state.changedFiles = [];
  ...
  saveState();
  return { ok: true, message: "Changes approved. New baseline saved." };
}

function revertChanges() {
  console.log("[guardian] Resetting changes...");
  ...
  if (restored) {
    ...
    saveState();
    reloadHA();
    return { ok: true, message: "Changes reset. HA reload triggered." };
  }
...
function crashRecovery() {
  ...
  if (state.status === "pending") {
    if (!state.deadline) {
      console.log("[guardian] Startup-drift approval is pending; crash recovery will not auto-reset it");
      return;
    }
    console.log(
      "============================================================"
    );
    console.log("  CRASH RECOVERY: Unapproved changes found - resetting!");
    console.log(
      "============================================================"
    );
    if (fs.existsSync(BACKUP_DIR) && restoreBackup()) {
      ...
      console.error("[guardian] Crash recovery reset failed; keeping pending state for manual intervention");
    }
  }
}
...
function initializeBaseline() {
  ...
  if (!ensureBaselineManifest()) {
    if (fs.existsSync(BASELINE_FILE) || directoryHasEntries(BACKUP_DIR)) {
      console.error("[guardian] Existing baseline is invalid; refusing to overwrite it without approval");
    ...
    console.log("[guardian] No baseline found; creating initial backup...");
    if (!createBackup()) {
      state.status = "idle";
      state.deadline = null;
      state.changedFiles = [];
  ...
  if (currentHash && backupHash && currentHash !== backupHash) {
    console.log("[guardian] Existing config differs from last approved baseline; approval required without auto-reset");
    state.status = "pending";
    state.deadline = null;
    state.changedFiles = getChangedFiles();
    state.lastHash = currentHash;
  ...
  console.log("[guardian] Existing baseline matches current config");
  state.status = "idle";
  state.deadline = null;
  state.changedFiles = [];
  state.lastHash = currentHash;
    ...
    try {
      ...
      if (state.status === "idle") {
        ...
        if (state.deadline && Date.now() > state.deadline) {
          console.log("[guardian] ⏰ TIMEOUT — resetting all changes");
          revertChanges();
          return;
        }
        saveState();
...
function getIngressPath(req) {
  if (!req || !req.headers["x-ingress-path"]) return "";
  const value = String(req.headers["x-ingress-path"]).replace(/\/$/, "");
  return /^\/api\/hassio_ingress\/[A-Za-z0-9_-]+$/.test(value) ? value : "";
}
...
<body>
  <div class="wrap">
    ...
    <div class="card">
      ...
      ${
        isPending
          ? `<div class="timer ${remaining != null && remaining < 120000 ? "urgent" : ""}">${remaining == null ? "Review" : `${min}:${String(sec).padStart(2, "0")}`}</div>
             <div class="timer-sub">${remaining == null ? "approval required; automatic reset paused" : "until automatic reset"}</div>
             ${
               filesList
                 ? `<div class="files">
             ...
             <div class="actions">
               <form method="POST" action="${ingressPath}/__guardian__/api/confirm" style="flex:1">
                  <button class="btn btn-confirm" type="submit">Approve changes</button>
                </form>
                <form method="POST" action="${ingressPath}/__guardian__/api/revert" style="flex:1">
                  <button class="btn btn-revert" type="submit">Reset changes</button>
                </form>
             </div>`
          : `<div class="empty">
               <div class="empty-emoji">✨</div>
               <h2>No pending changes</h2>
               <p>Your config is in a known-good state. Edits in OpenCode will appear here for approval.</p>
             </div>`
      }
    </div>
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === "/__guardian__/api/confirm" && req.method === "POST") {
    ...
    if (accept.includes("text/html")) {
      res.writeHead(303, { Location: `${getIngressPath(req)}/__guardian__/` });
      res.end();
    } else {
...
// Boot Sequence
// ---------------------------------------------------------------------------
console.log("[guardian] Recovering interrupted backup transactions...");
recoverBackupTransaction();

console.log("[guardian] Running crash recovery check...");
crashRecovery();

console.log("[guardian] Checking approved config baseline...");
initializeBaseline();
